...
def build_route_explanation(
    *,
    ...
    user_prompt = f"""
Ты — экспертный гид по культурным маршрутам России. Тебе передан готовый маршрут, уже оптимизированный по времени и интересам пользователя.

Данные маршрута (JSON):
{json.dumps(payload, ensure_ascii=False, indent=2)}

Сформулируй короткое, связное описание на чистом русском языке, без списков, без Markdown и без выделений. Дай 1–2 компактных абзаца суммарно 80–120 слов.

Обязательно явно скажи, что маршрут укладывается в отведённый временной лимит. Если передана примерная длительность маршрута, укажи, что есть запас по времени; если точных чисел нет, не приводи цифры и просто отметь соответствие лимиту.

Кратко раскрой соответствие основным интересам (теги переведены на русский) и объясни логику порядка точек как естественное движение по городу. Пиши только сплошной текст. Не выдумывай факты.
""".strip()
